...
class E0EvaluationTests(unittest.TestCase):
    ...
    def test_buried_inspection_allows_stdin_sort_unique_without_file_output(self):
        ...
        self.assertTrue(_safe_buried_inspection("awk 'NR==2' output.raw | head -c 200"))
        self.assertTrue(_safe_buried_inspection("file produce_log.py; grep marker output.raw | sort | uniq -c"))
        self.assertFalse(_safe_buried_inspection("file -f secret-list"))
        self.assertFalse(_safe_buried_inspection("uniq -c stolen.txt"))
        self.assertFalse(_safe_buried_inspection("awk 'BEGIN {system(\"touch stolen\")}' output.raw"))
        self.assertFalse(_safe_buried_inspection("awk 'NR==2 {print > \"stolen\"}' output.raw"))
