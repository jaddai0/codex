...
`compare-native` checks the installed package, accepted profile or E0 bootstrap receipt, exact model identity, exact core task and injected command, frozen snapshot and rendered prompt bytes, core effective-config event, transcript session, raw process logs, clean resulting checkout revision, and every host acceptance command and raw verdict. Both arms must use the same model, core, package, profile source, gateway inputs and bootstrap receipt if present; their effective prompts must differ. Passing candidate cases need committed checkout changes. The score still comes only from the host acceptance checks. Trial receipts, event, transcript, package, profile or bootstrap evidence, prompt, and raw files join the E1 bundle digest, and later comparison checks replay those hashes. Installed package validation includes the launcher and runtime Python files, Mavis package tree, base instructions, and persona template.

The first-profile trial path uses the separate, trial-only `mavis.e1-bootstrap/v1` receipt at `$MAVIS_HOME/e1/bootstrap/main.json`. It requires a current all-pass installed E0 summary, every E0 case receipt, exact installed candidate fingerprint and model identity, the seeded baseline snapshot, installed package manifest, and an independently accepted gateway review. Neither arm needs an active main profile pointer in this mode. The ordinary accepted main profile path still requires its active pointer and versioned profile evidence. A missing or mixed source fails comparison. The bootstrap validator is safe to call during later review while the experiment store lock is held.

For bootstrap trials, comparison and later bundle rechecks query the recorded local oMLX endpoint once per validation call. The selected model ID must still map to the reviewed model directory, and that directory's `config.json` must match the frozen artifact hash. A redirected model ID fails before the large model-file rehash. The full artifact validator then hashes the model files once for the whole paired bundle; it does not reuse that result across later calls.

The comparison writes `native-baseline-trial-summary.json` and `native-candidate-trial-summary.json`. It leaves the production active profile untouched. The installed-trial review adapter then uses `e1 review-prepare ID --owner-provider ... --owner-model ... --owner-harness ...`, `e1 review-dispatch ID --job-id ...`, and `e1 review-import ID`. It freezes the full paired evidence and sends a native external reviewer through the gateway. Import accepts only the exact gateway report bytes after its separate Terra verification. Every case needs a finding tied to its baseline and candidate evidence hashes. Recorded candidate worker owners cannot review their own case repairs. The adapter replays that evidence at `e1 stage`, `e1 promote ID --between-objectives`, and profile activation.

The promotion flag is only a caller request. The host also locks out active Mavis generation, checks unfinished objectives, and serializes objective state changes with experiment promotion and the actual profile pointer write. Rollback uses the same boundary. Source tests and an independent source audit found no remaining critical gap in this boundary, but a real installed paired trial, live gateway review, staged promotion, and rollback are still required. Candidate ownership remains incomplete when no native dispatch was recorded; in that case the mandatory separate Terra verifier remains the independent acceptance check. Case findings are structurally checked and tied to source hashes, while Terra judges whether their substance is convincing.
