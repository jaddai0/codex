...
from prepare_runtime import atomic_write
...
def launch(receipt_path: Path | None, argv: list[str]) -> int:
    ...
    if receipt_path is not None:
        if any(argument in {"--model", "-m"} or argument.startswith("--model=") or
               re.match(r"^(-c\s*)?\s*model\s*=", argument) for argument in argv[1:]):
            raise ValueError("model override conflicts with accepted main profile")
